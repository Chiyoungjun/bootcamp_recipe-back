...
def get_bmi_recommendations(user_id: str = Query(...), db: Session = Depends(get_db)):
    ...
    BMI_CATEGORY_MAP = {
        "Extreme Obesity": ["무침·샐러드·나물", "볶음·조림"],       # 고도비만: 저칼로리, 건강식 위주
        "Obesity": ["무침·샐러드·나물", "볶음·조림"],             # 비만: 저칼로리, 건강식 위주
        "Overweight": ["무침·샐러드·나물", "볶음·조림"],           # 과체중
        "Normal": [],                                            # 정상: 전체 추천
        "Weak": ["밥·죽", "구이·찜·오븐", "국·찌개·탕"],           # 저체중: 영양가 포함, 국·찌개·탕 추가
        "Extremely Weak": ["밥·죽", "구이·찜·오븐", "국·찌개·탕"],  # 극저체중: 고영양 집중, 국·찌개·탕 포함
    }
